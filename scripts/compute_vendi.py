...
dataset = 'ag_news'
embeddings_file = Path('embeddings/ag_news_glove.npy')
...
def _row(condition, label, n_docs, seed, vs, mean_cos, run_id):
    return {
        'run_id':               run_id,
        'dataset':              dataset,
...
def per_topic_diversity(doc_vectors, meta, run_id):
    ...
    for topic in topics:
        ...
        for n_docs in n_docs_values:
            ...
            for seed in seeds:
                ...
                results.append(_row('per_topic', topic, n_docs, seed, vs, mean_cos, run_id))
                print(f"[per_topic] {topic:>10s} n={n_docs:<6d} seed={seed} "
                      f"VS={vs:.3f} mean_cos={mean_cos:.3f}")
    return results


def n_topics_diversity(doc_vectors, meta, run_id, docs_per_topic=500):
    ...
    for k in n_topics_values:
        ...
        for seed in range(n_seeds):
            ...
            results.append(_row('n_topics', label, len(X), seed, vs, mean_cos, run_id))
...
assert len(doc_vectors) == len(meta), "embeddings and meta must align row-wise"

run_id = datetime.now().strftime('%Y%m%d_%H%M%S')

results = per_topic_diversity(doc_vectors, meta, run_id)
results += n_topics_diversity(doc_vectors, meta, run_id)

results_file.parent.mkdir(parents=True, exist_ok=True)
df_out = pd.DataFrame(results)
df_out.to_csv(results_file, mode='a', header=not results_file.exists(), index=False)
print(f"\nSaved {len(results)} rows to {results_file} (run_id={run_id})")
